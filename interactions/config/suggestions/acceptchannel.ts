...
export async function button(i: MessageComponentInteraction) {
	...
	if (i.customID == "cfg-act-chnl") {
		await i.message.edit({
			embeds: [
				new Embed({
					...createEmbedFromLangData(
						lang,
						"interactions.config.suggestions.pickacceptchannel",
					),
					author: {
						name: "Bidome bot",
						icon_url: i.client.user!.avatarURL(),
				...
				if (response.content.toLowerCase() == "none") {
					...
					if (missingPerms.length > 0) {
						...
						await supabase
							.from("servers")
							.update({
								suggestion_accepted_channel: channel.id,
							})
							.eq("server_id", i.guild!.id);
						...
						await i.message.edit({
							embeds: [
								new Embed({
									...createEmbedFromLangData(
										lang,
										"interactions.config.suggestions.setacceptchannel",
										`<#${channel.id}>`,
									),
									author: {
										name: "Bidome bot",
