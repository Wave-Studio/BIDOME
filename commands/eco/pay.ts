			...
			if (isNaN(number) && !allRegex.test(ctx.argString)) {
				await ctx.message.reply(undefined, {
					embed: new Embed({
						...
						title: 'Bidome eco',
						description: `That's not a valid number! Please provide a valid number`,
					}).setColor('random'),
				});
			} else {
				if (number < 0) {
					...
					if (profile.balance < number) {
						await ctx.message.reply(undefined, {
							embed: new Embed({
								...
								title: 'Bidome eco',
								description: `You need to actually have that much money!`,
							}).setColor('random'),
						});
					} else {
						const message = await ctx.message.reply(undefined, {
							embed: new Embed({
								...
								title: 'Bidome eco',
								description: `Mention a user to send $${number} to or type \`cancel\` to cancel!`,
								footer: {
								...
								if (!target) {
									return await message.edit(undefined, {
										embed: new Embed({
											...
											title: 'Bidome eco',
											description: `Unable to find target user!`,
										}).setColor('random'),
									});
								} else {
									...
									if (userProfile.balance < number) {
										...
										await saveProfile(ctx.guild.id, userProfile);
										await saveProfile(ctx.guild.id, targetProfile);
										return await message.edit(undefined, {
											embed: new Embed({
												author: {
													name: 'Bidome bot',
												...
												title: 'Bidome eco',
												description: `Sent ${number} to ${target.tag}!`,
											}).setColor('random'),
										});
									}
								}
