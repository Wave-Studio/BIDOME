// From: https://github.com/denoland/fresh/pull/1966

import { JSX, options as preactOptions, VNode } from "preact";

import { UnoGenerator, type UserConfig } from "@unocss/core";
import type { Theme } from "@unocss/preset";
...
export function installPreactHook(classes: Set<string>) {
	...
	(preactOptions as PreactOptions).__b = (
		...
		if (typeof vnode.type === "string" && typeof vnode.props === "object") {
			const { props } = vnode;
			// @ts-expect-error No clue why this errors, they just wrote it this way ig - Bloxs
			if (props.class) {
				// @ts-expect-error No clue why this errors, they just wrote it this way ig - Bloxs
...
export default function unocss({
	...
	return {
		...
		async buildStart() {
			...
			// Prevent file not found error - Bloxs
			await Deno.mkdir(join("./dashboard/_fresh", "static"), {
				recursive: true,
			});

			await Deno.writeTextFile(
				// Hack to get this to work with our directory structure - Bloxs
				join("./dashboard/_fresh", "static", "uno.css"),
				unoResetCSS + css,
			);
		},
	};
