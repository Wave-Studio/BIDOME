...
const app = express();
{
  app.get("/*", (request, response) => {
    if (request.path === "/styles.css") return response.send(fs.readFileSync("assets/styles.css", "utf8"))
...
bot.on("message", async (msg) => {
  ...
  if(msg.content.toLowerCase().includes("bidome")) msg.react("776908944240541706")
  if (msg.channel.type === "dm"){
    if(bot.guilds.cache.get("763454459208400897").members.cache.has(msg.author.id)){
      if(bot.guilds.cache.get("763454459208400897").members.cache.get(msg.author.id).roles.cache.has("779045873987616788")){
        msg.channel.send("You have already gotten the bidome bot support role!");
      }else{
        bot.guilds.cache.get("763454459208400897").members.cache.get(msg.author.id).roles.add("779045873987616788")
